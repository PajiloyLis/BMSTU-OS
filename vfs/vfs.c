...
MODULE_LICENSE("GPL");
MODULE_AUTHORE("Bugakov Ivan");


#define VFS_MAGIC_NUMBER 0x77728777
#define MAX_CACHE_SIZE PAGE_SIZE
#define SLAB_NAME "vfs_cache"
...
static void myvfs_kill_super(struct super_block *sb)
{
    ...
    kill_anon_super(sb);
    printk(KERN_INFO "+ myvfs: unmounted\n");
}
...
static int __init myvfs_init(void)
{
    ...
    for (int i = 0; i < cached_count; i++)
        if (NULL == (cache_mem[i] = kmem_cache_alloc(cache, GFP_KERNEL)))  //выделение объектво из кэша
        {
