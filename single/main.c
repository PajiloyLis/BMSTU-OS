...
#define PROC_DIR "seq_dir"
#define PROC_FILE "seq_file"
#define PROC_LINK "seq_link"
#define SIZE 16
...
static ssize_t proc_file_write(struct file* file, const char __user* buf, size_t count, loff_t *ppos)
{
    char pid_buf[16];
    int pid;
    printk(KERN_INFO "+++ call single_write\n");
...
static ssize_t my_seq_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
...
static int show(struct seq_file *m, void *v)
{
    ...
    for(int i = 0; i < pid_count; ++i) {
        task = pid_task(find_vpid(pid_array[i]), PIDTYPE_PID);
        if (!task) {
            seq_printf(m, "Error pid task for pid %d");
            return 0;
        }
        seq_printf(m, "+++ pid %lld\ncomm %s\nstatic_prio %d\nflags %x\n", pid_array[i], task->comm, task->static_prio,
                   task->flags);
    }
    return 0;
}
...
static int __init md_init(void)
{
    ...
    printk(KERN_INFO "+++ single module\n");
    return 0;
}
